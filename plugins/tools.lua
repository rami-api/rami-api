...
local function run(msg, matches)
  ...
  if is_sudo(msg) then
	if matches[1]:lower() == "ارسال ملف" and matches[2] and matches[3] and msg.reply_to_message then
		...
		sendDocument(msg.to.id, send_file, msg.id, "@Xxx_DEVRAMI_xxX")
	end
	if matches[1]:lower() == "اجلب ملف" and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		sendDocument(msg.to.id, plug, msg.id, "@Xxx_DEVRAMI_xxX")
    end
...
_ﭑۇﭑمہۣۧڔٌ ﭑلہۣۧمہۣۧطہۣۧۇڔٌ لہۣۧسہۣۧۇڔٌسہۣۧ DEV RAMIۣۧ_
