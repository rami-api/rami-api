--Begin msg_checks.lua By @RAMBO_SYR
...
local auto_leave = 'AutoLeaveBot'
  ...
  send_msg(msg.to.id, "انها ليست احدى مجموعاتي ☹️\nراسل المطور لاضافتي 😋", nil, "md")
  leave_group(chat)
      end
   end
end
...
return
   ...
   del_msg(chat, msg.id)
    kick_user(user, chat)
   del_msg(chat, msg.id)
send_msg(chat, "المستخدم🐥 "..user_name.." `[ "..user.." ]` _تم طرده بسبب_ *التكرار*", nil, "md")
